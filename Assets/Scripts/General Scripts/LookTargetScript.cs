...
public class LookTargetScript : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        if (lookTarget == silRb)
        {
           ...
           selfRb.position = Vector2.SmoothDamp(selfRb.position, new Vector2(silRb.position.x + sil.GetComponent<SilControl>().look.x * multiplier, silRb.position.y + sil.GetComponent<SilControl>().look.y * multiplier), ref currentSpeed, moveSpeed);
