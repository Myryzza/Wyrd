...
//remove coyote time
//move camera aiming to the right stick
//make a pause menu
...
public class SilControl : MonoBehaviour
{
    ...
    public Image[] lifeCells;
    public Sprite fullCell;
    public Sprite emptyCell;

    //Environment Variables
    [Header("Environment Variables")]
    public float bouncerKbMod;
    public float bouncerKbModx;
    [HideInInspector] public bool redFlowerToggle;
    [HideInInspector] public bool greenFlowerToggle;
    ...
    private void Awake()
    {
        ...
        //input.Normal.LookRight.performed += LookRightPerformed;
        //input.Normal.LookRight.canceled += LookRightCanceled;
        input.Normal.LookRight.performed += context => lookRight = context.ReadValue<float>();
        input.Normal.LookRight.canceled += context => lookRight = 0;

        //input.Normal.LookLeft.performed += LookLeftPerformed;
        //input.Normal.LookLeft.canceled += LookLeftCanceled;
        input.Normal.LookLeft.performed += context => lookLeft = context.ReadValue<float>();
        input.Normal.LookLeft.canceled += context => lookLeft = 0;

        //input.Normal.LookUp.performed += LookUpPerformed;
        //input.Normal.LookUp.canceled += LookUpCanceled;
        input.Normal.LookUp.performed += context => lookUp = context.ReadValue<float>();
        input.Normal.LookUp.canceled += context => lookUp = 0;

        //input.Normal.LookDown.performed += LookDownPerformed;
        //input.Normal.LookDown.canceled += LookDownCanceled;
        input.Normal.LookDown.performed += context => lookDown = context.ReadValue<float>();
        input.Normal.LookDown.canceled += context => lookDown = 0;
    ...
    private void DashPerformed(InputAction.CallbackContext obj)
    {
        ...
        if (dashCount > 0)
        {
            ...
            if (greenFlowerToggle == true)
            {

                greenFlowerToggle = false;

            }
            else if (greenFlowerToggle == false)
            {

                greenFlowerToggle = true;
    ...
    */
    ...
    private void JumpPerformed(InputAction.CallbackContext obj)
    {
        ...
        if ((onFloor == true || floorCoyoteTime > 0) || ((onWall == true || wallCoyoteTime > 0) && climbDigClaws >= 1))
        {
            ...
            if (redFlowerToggle == true)
            {

                redFlowerToggle = false;

            }
            else if (redFlowerToggle == false)
            {

                redFlowerToggle = true;
    ...
    */
    ...
    private void Update()
    {
        ...
        for (int i = 0; i < lifeCells.Length; i++)
        {
            ...
            if (i < currentLife)
            {
                lifeCells[i].sprite = fullCell;
            } else
            {
                lifeCells[i].sprite = emptyCell;
            }
        ...
        */
        ...
        dashDir = new Vector2((right - left), /*(up - down)*/ 0);
        ...
        if (tether > 0)
        {
            if (canBash == true && bashTarget != null && hasBash == true)
            {
    ...
    void FixedUpdate()
    {
        ...
        if (silRb.velocity.x > 0.001) //something weird going on here, player drifts right after ending left input, though this might be due to stick drift
        {
            silRb.velocity = new Vector2(silRb.velocity.x - walkDecel, silRb.velocity.y);
        }
        if (silRb.velocity.x < -0.001)
        {
            silRb.velocity = new Vector2(silRb.velocity.x + walkDecel, silRb.velocity.y);
        }
        #endregion
        ...
        if (startDash == true)
        {
            ...
            silRb.velocity = new Vector2(dashDir.x * dashSpeed, dashDir.y * dashSpeed * bashVOffset);
            startDash = false;
        }
        //end dash
        if (endDash == true)
    ...
    private void OnCollisionEnter2D(Collision2D collision)
    {
        ...
        if (collision.gameObject.tag == "Bouncer")
        {
            ...
            silRb.velocity += new Vector2((silRb.position.x - collision.rigidbody.position.x) * bouncerKbMod * bouncerKbModx, (silRb.position.y - collision.rigidbody.position.y) * bouncerKbMod);
            airJumpCount = multiJump;
            dashCount = maxDash;
